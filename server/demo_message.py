#!/usr/bin/env python
"""
Demo script to visualize the streaming output from MessageInstructor.
"""
import asyncio
import sys
import time
...
logger = logging.getLogger("MessageDemo")
...
from agent.instructors import MessageInstructor


async def demo_message_instructor():
    """Demonstrate the streaming behavior of MessageInstructor."""
    
    # Create a MessageInstructor instance
    instructor = MessageInstructor()
    
    # A question that should generate a detailed response
    query = "Explain the key principles of artificial intelligence and machine learning in a clear, structured way."
    
    print("\n💬 MessageInstructor Streaming Demo 💬")
    print("=" * 80)
    print(f"Query: {query}")
    print("=" * 80)
    print("\nStreaming message response (watch how it updates):\n")
    ...
    async for chunk in instructor.generate(query):
        ...
        logger.info(f"Received chunk #{chunk_count}: message_text={message_text}, message_length={len(message_text)}")
...
if __name__ == "__main__":
    asyncio.run(demo_message_instructor()) 